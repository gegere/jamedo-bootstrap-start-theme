...
function skematik_services_nav($menu_class='') {
	// display the wp3 menu if available
    wp_nav_menu( 
    	array( 
    		'menu' => 'services_nav', /* menu name */
    		'menu_class' => empty($menu_class)?'nav navbar-nav':$menu_class,// nav navbar-nav
    		'theme_location' => 'services_nav', /* where in the theme it's assigned */
    		'container' => 'false', /* container class */
    		'depth' => '2', /* suppress lower levels for now */
    		'walker' => new description_walker()
    	)
    );
do_action( 'skematik_services_nav' );
}
